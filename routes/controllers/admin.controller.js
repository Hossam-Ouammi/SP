...
async function mettreAJourTarifCompteUtilisateur(req, res) {
  ...
  if (
    ...
    return res.status(400).json({
      message:
        "Compte cible, tarif horaire entier entre 0 et 5000, et mot de passe actuel obligatoires.",
    });
  }
  ...
  if (Number(compteCible.est_admin) === 1) {
    return res.status(400).json({
      message: "Le tarif d'un administrateur n'est pas configurable ici.",
    });
  }
  ...
  return res.json({
    message: `Le tarif horaire de ${compteCible.nom} est maintenant de ${tarifNormalise} dh.`,
  });
}
